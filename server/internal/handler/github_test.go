...
// TestWebhook_RoutesToProjectResourceRepoOwningWorkspace covers workspaces
// whose durable repo context is stored on a project resource rather than the
// legacy workspace.repos registry. PR auto-linking must still find the issue in
// that workspace.
func TestWebhook_RoutesToProjectResourceRepoOwningWorkspace(t *testing.T) {
	if testHandler == nil {
		t.Skip("handler test fixture not initialized (no DB?)")
	}
	ctx := context.Background()
	secret := "route-by-project-resource-secret"
	t.Setenv("GITHUB_WEBHOOK_SECRET", secret)

	w := httptest.NewRecorder()
	req := newRequest("POST", "/api/issues?workspace_id="+testWorkspaceID, map[string]any{
		"title":  "route-by-project-resource test",
		"status": "in_progress",
	})
	testHandler.CreateIssue(w, req)
	if w.Code != http.StatusCreated {
	...
	const repoOwner, repoName = "project-acme", "project-widget"
	...
	if _, err := testPool.Exec(ctx, `UPDATE workspace SET repos = '[]'::jsonb WHERE id = $1`, testWorkspaceID); err != nil {
	...
	if err := testPool.QueryRow(ctx, `
		...
		VALUES ($1, 'project-resource routing', 'planned', 'none')
		RETURNING id
	...
	`, projectID, testWorkspaceID, []byte(`{"url":"`+repoURL+`"}`)); err != nil {
		t.Fatalf("create project resource: %v", err)
	}

	testPool.Exec(ctx, `DELETE FROM workspace WHERE slug = $1`, "project-resource-install-ws")
	wsA, err := testHandler.Queries.CreateWorkspace(ctx, db.CreateWorkspaceParams{
		Name:        "project-resource-install-ws",
		Slug:        "project-resource-install-ws",
		IssuePrefix: "PRW",
	})
	if err != nil {
		t.Fatalf("CreateWorkspace: %v", err)
	...
	body := map[string]any{
		...
		"pull_request": map[string]any{
			"number":     prNumber,
			"html_url":   "https://github.com/" + repoOwner + "/" + repoName + "/pull/4243",
			"title":      "Implement thing " + created.Identifier,
			"body":       "Issue: " + created.Identifier,
			"state":      "open",
			"draft":      false,
			"merged":     false,
			"created_at": "2026-04-28T00:00:00Z",
	...
	if len(linked) != 1 {
		t.Fatalf("expected 1 linked PR on the project-resource workspace issue, got %d", len(linked))
	}
}
